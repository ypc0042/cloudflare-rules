...
1. **壳**：DNS（含 `proxy-server-nameserver`）、端口、`ipv6: false`、自动选择测速 URL 与可用完整模板对齐。  
2. **系统组（固定）**：手动选择、自动选择、各地区（本站词表+互斥）、其他地区、全球直连、漏网之鱼。  
3. **业务组（动态）**：仅根据你在订阅集里**勾选的规则分类**生成代理组 + `RULE-SET`，**不**内置参考模板里写死的 GitHub/苹果/微软等组。  
4. **节点**：不生成节点列表；`proxy-providers.机场订阅.url` 自填或客户端合并。  
5. **基础规则**：`private` / `cn` 直连 + 勾选分类的 RULE-SET + `MATCH` 漏网。
